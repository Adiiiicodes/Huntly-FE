...
export const getSavedProfiles = async (userId: string): Promise<SavedProfile[]> => {
  try {
    ...
    console.log(`Fetching saved profiles for user: ${userId}`);
    
    // Use relative URL to leverage Next.js API proxy
    const response = await fetch(`/api/profiles/save/${userId}`, {
      method: 'GET',
      headers: {
...
export const removeProfile = async (candidateId: string): Promise<ProfileResponse> => {
  try {
    ...
    // Use relative URL to leverage Next.js API proxy
    const response = await fetch(`/api/profiles/remove/${candidateId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
